...
object ServerExceptions:
  ...
  case class Unauthorized(message: String = InvalidCredentialsMsg)
  ...
  case class ValidationError(errors: Map[String, String])
      extends RuntimeException(s"Validation errors: ${ errors.mkString(",") }")
